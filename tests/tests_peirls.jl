function test_peirls(p::Parameters, l::Lattice)
  const L = l.L
  ...
  for f in 1:2
    for s in 1:2
      ...
      println(s," ",f)
      @assert all( isapprox.(fluxes_nn, exp(im * B[s,f])) )
      @assert all( isapprox.(fluxes_nnn_1, exp(im * B[s,f] / 2)) )
      @assert all( isapprox.(fluxes_nnn_2, exp(im * B[s,f] / 2)) )
      @assert all( isapprox.(fluxes_nnn_3, exp(im * B[s,f] / 2 )) )
      @assert all( isapprox.(fluxes_nnn_4, exp(im * B[s,f] / 2)) )
      @assert all( isapprox.(fluxes_nnnn, exp(im * 4 * B[s,f])) )
      @assert all( isapprox.(fluxes_mixed_1, exp(im *  B[s,f])) )
      @assert all( isapprox.(fluxes_mixed_2, exp(im *  B[s,f])) )
    end
  end
  return true
end